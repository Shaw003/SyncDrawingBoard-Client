...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *,id> *)change context:(void *)context
{
    ...
    if (object == self.paintView) {
        ...
        [self.client sendObject:allLines];
    }
}
- (void)dealloc {
    [self.client stop];
...
{
    //显示连接的数量
    NSString *str = [NSString stringWithFormat:@"已连接到%ld个服务器端", self.client.connections.count];
    LogBlue(@"%@", str);
    if (self.client.connections.count) {
        
        UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"连接信息" message:str preferredStyle:UIAlertControllerStyleAlert];
...
{
    //显示已经连接到哪个服务器
    LogGreen(@"客户端已经连接到名为%@的服务器端", connection.netService.name);
    //更新状态
    [self updateStatus];
}
...
{
    //显示失联日志
    LogRed(@"客户端断开与%@的连接", connection.netService.name);
...
- (void)client:(AsyncClient *)theClient didReceiveCommand:(AsyncCommand)command object:(id)object connection:(AsyncConnection *)connection;
{
    ...
    if ([object isKindOfClass:[NSArray class]]) {
        ...
        LogPurple(@"客户端接收到服务器端发送的Line数组,数组长度%ld", receivedArray.count);
        NSMutableArray *allReceivedlines = [object mutableCopy];
        self.paintView.receivedLines = [allReceivedlines mutableCopy];
